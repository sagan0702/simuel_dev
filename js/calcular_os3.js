...
function calcularOS() {
    ...
    //  } else {
        ...
        //alert(`  Qust:  ${qust} , Qgaest: ${qgaest}  , Qinfra:  ${qinfra}  `);
       
        //Quantidade de dias uteis do período:
        //var qdperiodo = inicio - fim
        ...
        qtde_dias_disponiveis.innerHTML = `${qddisp}`
                
        //Quantidade de Grupo de Atividades (GA) estimadas: 
        var qgaest = (nturnas + ntbat)
        qtde_ga_estimadas.innerHTML = `${qgaest}`
                
        //Quantidade de UST:  
        var qust =  (qgaest / qinfra) 
        qtde_ust.innerHTML = `${qust}`
...
function criarOS() {
    ...
    fim = $('#txt_fim').val();
    urnas = $('#txt_t_urnas').val();
    bat = $('#txt_t_baterias').val();
              
        if (n_local == '' || n_os == '' || inicio == '' || fim == '' || urnas == '' || bat == ''){
            alert("Preencha todos os campos.");
            return false;
        }
